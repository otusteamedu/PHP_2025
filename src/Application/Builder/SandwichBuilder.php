...
use Kamalo\BurgersShop\Domain\Entity\Sandwich;
...
class SandwichBuilder implements ProductBuilderInterface
{
    ...
    public function build(): Sandwich
    {
        $sandwichData = $this->config->get('sandwiches', $this->productCode);

        if (!$sandwichData) {
            throw new Exception('отсутствует рецепт сэндвича: ' . $this->productCode);
        }
        $this->name = $sandwichData['name'];
        $this->price = intval($sandwichData['price']) + $this->extraPrice;


        foreach ($sandwichData['ingredients'] as $code => $count) {
            $this->itemsIterator->add(
                new Item(
                    $code,
                    $count,
