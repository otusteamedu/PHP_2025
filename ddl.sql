...
(
    ...
    constraint unique_places_number_and_row_number unique (hall_id, number, row_number)
);
...
-- Добавил эти индексы, т.к. оказались наиболее эффективными
create index tickets_order_idx on tickets (order_id);
create index tickets_session_id_index on tickets (session_id);
create index sessions_begin_at_index on sessions (begin_at);

-- Функция случайной строки
create function random_str(length integer) returns text
    language sql as $$
