-- Добавление 10.000 тестовых данных
...
SELECT
    ...
    CASE
        ...
        END,
    round((88000000000 + random() * 9999999))::text,
...
SELECT
    ...
    CASE
        WHEN ss.id <= 5
            THEN date_trunc('day', CURRENT_TIMESTAMP) + interval '9 hours' + ((ss.id - 1) * 3) * interval '1 hour'
        ELSE date_trunc('day', CURRENT_TIMESTAMP) + interval '1 day 9 hours' + ((ss.id - 6) * 3) * interval '1 hour'
        END,
    CASE
        WHEN ss.id <= 5
            THEN date_trunc('day', CURRENT_TIMESTAMP) + interval '9 hours' + ((ss.id - 1) * 3) * interval '1 hour' + round(80 + random() * 100)::integer * interval '1 minute'
        ELSE date_trunc('day', CURRENT_TIMESTAMP) + interval '1 day 9 hours' + ((ss.id - 6) * 3) * interval '1 hour' + round(80 + random() * 100)::integer * interval '1 minute'
        END,
    round((1000 + random() * 1000))::integer,
    round(1 + random() * 4)::integer
FROM generate_series(1, 5) AS hs(id)
    JOIN generate_series(1, 10) AS ss(id) ON true;
...
FROM generate_series(1, 50) AS ss(id)
         JOIN generate_series(1, 4) AS ts(id) ON true;

-- Генерация 10,000 тестовых записей о продажах билетов
WITH cte AS (
    SELECT
        ss.id AS screening_id,
        ts.id AS tid,
        round((1 + random() * 2999))::integer AS customer_id,
        timestamp '2025-01-01 15:00' + (20 * random())::integer * interval '1 day' AS  purchase_date
    FROM generate_series(1, 50) AS ss(id)
