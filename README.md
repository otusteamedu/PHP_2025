...
Для индекса idx_ticket_session_id был выбран тип b-tree, поскольку в запросе используется равенство:`join cinema.movie as m on m.id = s.movie_id`. B-tree поддерживает операторы: `< <= = >= >`.

Для таблицы cinema.session добавил в индекс поле start_time (для ускорения поиска), в include добавил: id, movie_id.
Для индекса idx_session_start_time был выбран тип b-tree, поскольку в запросе используется сравнение в диапазоне (<, >=): `where s.start_time >= date_trunc('week', now()) and  s.start_time < date_trunc('week', now()) + interval '1 week'`. B-tree поддерживает операторы: `< <= = >= >`.
...
Для индекса idx_orders_created_at_id был выбран тип b-tree, поскольку в запросе используется сравнение в диапазоне (<, >=): `where o.created_at >= date_trunc('week', now()) and  o.created_at < date_trunc('week', now()) + interval '1 week'`. B-tree поддерживает операторы: `< <= = >= >`.
...
Для индекса idx_ticket_order_id был выбран тип b-tree, поскольку в запросе используется равенство:` join cinema.orders as o on o.id = t.order_id`. B-tree поддерживает операторы: `< <= = >= >`.
...
Для индекса idx_place_hall_id был выбран тип b-tree, поскольку в запросе используется равенство: `join hall h on p.hall_id = h.id`. B-tree поддерживает операторы: `< <= = >= >`.
