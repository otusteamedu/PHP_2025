...
Для таблицы cinema.session добави в индекс поле start_time (для ускорения поиска), в include добавил: id, movie_id.
...
Добавил в индекс таблицы cinema.place поле hall_id. При помощи индекса удалось избавиться от вложенного цикла. Теперь заказы берутся из индекса, теперь не нужно делать полный проход по таблице orders.
...
В сотни раз уменьшилась стоимость. Теперь для join не приходится пробегаться по таблице в поиске нужных строк, данные берутся из индекса, что уменьшило стоимость запроса.

## Вывод

После добавления индексов снизилась оценка затрат на получение данных для сложных запросов. Индексация таблиц позволяет оптимизировать запрос за счет уменьшения времени получения данных.